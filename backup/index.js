...
class McBot
{
	constructor(name, ip, port) 
	{
		...
		this.bot.once('login', async () =>
		{
			...
			{
				let dt = 1100 + Math.round(this.rand(0, 500));
	...
	doActivity(cmd)
	{
		...
		switch (cmd)
		{
			...
			case "D":
				...
				break;
			case "RMB":
				this.bot.activateEntity(bot.nearestEntity());
				break;
			case "Space":
			...
			case "Look":
				...
				break;
			case "Write":
				this.randTime = bot.time.timeOfDay + 5;
				this.write = true;
				break;
			case "Kill":
				this.kill = true;
			...
			case "Swap hands items":
				this.bot.equip(bot.heldItem, "off-hand");
				break;
			case "Q":
				if(bot.heldItem.count>0)
				{
					this.bot.tossStack(bot.heldItem);
				}
				break;
			case "Fish":
				this.fishing = true;
